...
// 管理员/超级管理员账号额度为无限(-1)，普通用户取自身额度或默认值
function getEffectiveAccountLimit(user) {
    if (!user) return DEFAULT_ACCOUNT_LIMIT;
    if (user.role === 'admin' || user.role === 'super_admin') return -1;
    if (user.accountLimit === -1) return -1;
...
function getAllUsers() {
    ...
    return users
        ...
        .map(u => ({
            ...
            role: u.role,
            card: u.card,
            accountLimit: getEffectiveAccountLimit(u)
        }));
}

// 含密码哈希的用户列表（仅超级管理员可调用，用于审计/备份）
